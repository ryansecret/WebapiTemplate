...
namespace Ryan.Core
{
    ...
    public class AppDomainTypeFinder : ITypeFinder
    {
        ...
        public string AssemblySkipLoadingPattern { get; set; } = "^System|^mscorlib|^Microsoft|^CppCodeProvider|^VJSharpCodeProvider|^WebDev|^Castle|^Iesi|^log4net|^NHibernate|^nunit|^TestDriven|^MbUnit|^Rhino|^QuickGraph|^TestFu|^Telerik|^ComponentArt|^MvcContrib|^AjaxControlToolkit|^Antlr3|^Remotion|^Recaptcha";
        ...
        public IEnumerable<Type> FindClassesEndWith(IEnumerable<string> appendfixs)
        {
            var result = new List<Type>();
            var assemblies = GetAssemblies();
            var enumerable = appendfixs as string[] ?? appendfixs.ToArray();
            foreach (var assembly in assemblies)
            { 
                var types = assembly.GetTypes();
                foreach (var type in types)
                {
                    foreach (var af in enumerable)
                    {
                        if (type.Name.EndsWith(af))
                        {
                           result.Add(type);
                           break;
                        }
