...
namespace Ryan.Utility.ModelBinders
{
    ...
    public class MvcActionValueBinder : DefaultActionValueBinder
    {
        ...
        /// <summary>
        ///     Default implementation of the <see cref="T:System.Web.Http.Controllers.IActionValueBinder" /> interface. This
        ///     interface is the primary entry point for binding action parameters.
        /// </summary>
        /// <param name="actionDescriptor">The action descriptor.</param>
        /// <returns>
        ///     The <see cref="T:System.Web.Http.Controllers.HttpActionBinding" /> associated with the
        ...
        public override HttpActionBinding GetBinding(HttpActionDescriptor actionDescriptor)
        {
            ...
            var parameters = actionDescriptor.GetParameters().ToArray();
            var binders = Array.ConvertAll(parameters, DetermineBinding);
        ...
        private HttpParameterBinding DetermineBinding(HttpParameterDescriptor parameter)
        {
            ...
            var provider = attr.GetModelBinderProvider(config);
            var binder = provider.GetBinder(config, parameter.ParameterType);

            // Alternatively, we could put this ValueProviderFactory in the global config.
            var vpfs = new List<ValueProviderFactory>(attr.GetValueProviderFactories(config))
            {
        ...
        private class MvcActionBinding : HttpActionBinding
        {
            ...
            public override Task ExecuteBindingAsync(HttpActionContext actionContext,
                CancellationToken cancellationToken)
            {
                ...
                if (content != null)
                {
                    var fd = content.ReadAsAsync<FormDataCollection>().Result;
                    if (fd != null)
                    {
